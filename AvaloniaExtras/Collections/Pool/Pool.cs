...
namespace AvaloniaExtras.Collections.Pool;
...
/// </summary>
[AutoInterface]
public class Pool<TBaseType> : IPool<TBaseType>
    where TBaseType : IPooled
{
    private readonly ConcurrentDictionary<Type, ConcurrentQueue<TBaseType>> _pool = [];
    ...
    public void Register<TDerivedType>(Func<TDerivedType> factory, int capacity = 1)
    ...
    public TDerivedType Get<TDerivedType>()
        where TDerivedType : TBaseType, new() => (TDerivedType)Get(typeof(TDerivedType));

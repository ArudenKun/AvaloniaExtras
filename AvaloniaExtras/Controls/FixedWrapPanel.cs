...
public class FixedWrapPanel : Panel, INavigableContainer
{
    ...
    ///
    /// </summary>
    public static readonly StyledProperty<int> ItemsPerLineProperty = AvaloniaProperty.Register<
        FixedWrapPanel,
        int
    >(nameof(ItemsPerLine), 3);
    ...
    protected override Size MeasureOverride(Size constraint)
    {
        ...
        for (int i = 0, count = Children.Count; i < count; i++)
        {
            ...
            if (
                MathUtilities.GreaterThan(
                    currentLineSize.Width + childSize.Width,
                    lineConstraint.Width
                )
            )
            {
